...
def cluster_manifold_in_embedding(hl, y, label_names=None):
    ...
    nmi = np.round(metrics.normalized_mutual_info_score(y, y_pred), 5)
    ari = np.round(metrics.adjusted_rand_score(y, y_pred), 5)
    db = metrics.davies_bouldin_score(hle, y_pred)
    s = metrics.silhouette_score(hle, y_pred, metric='euclidean')
    s_dbw = S_Dbw(hle, y_pred)
...
def plot(x, y, plot_id, names=None):
    ...
    l = plt.legend(bbox_to_anchor=(-.1, 1.00, 1.1, .5), loc="lower left", markerfirst=True,
                   mode="expand", borderaxespad=0, ncol=args.n_clusters + 1, handletextpad=0.01, prop={'size': 18})
